...
# Create mapping in pd.dataframe
dfm = []
...
ddft = conn.sql("SELECT * FROM dft").set_alias("ddft")
ddfm = conn.sql("SELECT * FROM dfm").set_alias("ddfm")

# Need field on representative in clock time or standard time?
